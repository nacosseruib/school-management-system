...
@section('pageHeaderTitle') {{ strToUpper(__('SubModule Route Management')) }} @endsection
        ...
        <div class="main-content">
          ...
          @csrf
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title" id="from-actions-multiple">{{ __('CREATE ROUTE SUBMODULE') }}</h4>
                            <hr /> 
                            ...
                            <div class="row offset-md-3">
                                <div class="col-md-6">
                                    ...
                                    <select class="form-control" autofocus required name="moduleName" id="moduleName">
                                        <option value="{{ ((isset($editSubModule)) ? $editSubModule->moduleID : '') }}"> {{ ((isset($editSubModule)) ? ($editSubModule->module_name) : 'Select') }} </option>
                            ...
                            <div class="row offset-md-1">
                                <div class="col-md-5 mt-2">
                                    ...
                                    <input class="form-control" required name="routeName" id="routeName" value="{{ ((isset($editSubModule)) ? $editSubModule->submodule_name : old('routeName')) }}">
            ...
            <div align="center" class="card-header col-md-12">
                <table style="font-size:13px;" class="table table-hover table-stripped table-responsive table-condensed"> 
                    ...
                    <tbody class="text-left">
                        ...
                        @forelse($allModuleLoop as $moduleKey=>$listModule)
                         <tr>
                            <th colspan="11" class="text-center text-info"><b>{{ strtoupper($listModule->module_name) }}</b></th>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td class="d-print-none">
                                <a href="{{ route('editSubModule', ['ID'=>$list->submoduleID]) }}"><i class="fa fa-edit"></i></a>
                            </td>
                            <td class="d-print-none">
                            ...
                            <!-- Delete Modal -->
                                <div class="modal fade text-left d-print-none" id="deleteModule{{$list->submoduleID}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel12" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header bg-danger white">
                                            <h4 class="modal-title" id="myModalLabel12"><i class="fa fa-trash"></i> {{ __('Delete SubModule URL!')}}  </h4>
                            ...
                            <!--end Modal-->
                        @empty
                            <tr><td colspan="11" class="text-danger">{{ __('No Submodule found!') }}</td></tr>
                        @endforelse

                        @empty
                            <tr><td colspan="11" class="text-danger">{{ __('No Module found!') }}</td></tr>
                        @endforelse
                        </tbody>
                    </table>
                    <div class="row">
                        <div align="right" class="col-xs-12 col-sm-12">
                            Showing {{($allModuleLoop->currentpage()-1)*$allModuleLoop->perpage()+1}}
                            to {{$allModuleLoop->currentpage()*$allModuleLoop->perpage()}}
                            of  {{$allModuleLoop->total()}} entries
                            <br />
                            <div class="hidden-print">{{ $allModuleLoop->links() }}</div> 
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header bg-success white">
                      <h4 class="modal-title" id="myModalLabel12"><i class="fa fa-tree"></i> {{ __('Update/Add New SubModule')}}  </h4>
                      <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                      </button>
                    </div>
